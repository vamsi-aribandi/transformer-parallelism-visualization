...
## 2026-09-17 — Session 4: all six strategies in the article

- Figure polish rounds (user feedback): speed slider (recorded pacing = 2×; a seconds→centiseconds bug had floored every beat at 150ms); highlight-syncs-with-animation in play mode; per-device dim legend (B×T×D annotated tensor) in a FIXED detail band that hands off to the collective's wire-level ring animation (bidirectional ring AllGather/ReduceScatter with growing accumulators and visible absorbs, RS+AG for AllReduce, shortest-path AllToAll, single-hop P2P); backward gelu now renders dTmp ⊙ gelu′(Tmp) with a forward citation; dW shadows pinned to their weights (they were riding the phase-change slide); under-tensor chips web-hidden.
- **All six strategies shipped in the article**: DP, FSDP, TP, CP, EP, PP — each a prose section + <tpviz-figure>. PP needed the `mark_step` recorder hooks (its tick-grouped playback bypasses play_step): one marked step per tick (captioned with every stage's work) + one per P2P send, forward and backward. Program shows PP ticks as "stage 0 runs mb2 · stage 1 runs mb1". Six timelines: 1.23 MB data, 1.31 MB page.
